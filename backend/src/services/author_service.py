...
class AuthorService:
    ...
    async def update_author(
        self, author_id: int, author_data: dict
    ) -> Optional[Author]:
        self._validate_dates(author_data)
    ...
    def _validate_dates(self, author_data: dict):
        if "date_birth" in author_data and author_data["date_birth"]:
            if isinstance(author_data["date_birth"], str):
                try:
                    author_data["date_birth"] = datetime.fromisoformat(
                        author_data["date_birth"].replace("Z", "+00:00")
                    )
                except ValueError:
        ...
        if "date_death" in author_data and author_data["date_death"]:
            if isinstance(author_data["date_death"], str):
                try:
                    author_data["date_death"] = datetime.fromisoformat(
                        author_data["date_death"].replace("Z", "+00:00")
                    )
                except ValueError:
