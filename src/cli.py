...
def createsuperuser(
    ...
    async def _create_superuser():
        async with async_session_maker() as session:
            ...
            if not password:
                password_input = Prompt.ask("🔑 [bold]Введите пароль[/bold]", password=True)
                confirm_password = Prompt.ask("🔑 [bold]Подтвердите пароль[/bold]", password=True)
            ...
            try:
                with console.status("[bold green]Создаю суперпользователя...[/bold green]"):
                ...
                table.add_row("✅ Статус", "Активен" if admin_user.is_active else "Неактивен")
    ...
    async def _deactivate_user():
        async with async_session_maker() as session:
            ...
            if not user.is_active:
                rprint(f"ℹ️ [yellow]Пользователь '{username}' уже деактивирован[/yellow]")
                return True
            
            if Confirm.ask(f"⚠️  Вы уверены, что хотите деактивировать пользователя [bold]{username}[/bold]?"):
...
def promote(username: str):
    ...
    async def _promote_user():
        async with async_session_maker() as session:
            ...
            if user.role == "admin":
                rprint(f"ℹ️ [yellow]Пользователь '{username}' уже администратор[/yellow]")
                return True
            
            if Confirm.ask(f"👑 Назначить пользователя [bold]{username}[/bold] администратором?"):
                user.role = "admin"
                await session.commit()
                rprint(f"✅ [green]Пользователь '{username}' теперь администратор[/green]")
                return True
            else:
                rprint("ℹ️ [yellow]Операция отменена[/yellow]")
                return False
...
def demote(username: str):
    ...
    async def _demote_user():
        async with async_session_maker() as session:
            ...
            if user.role != "admin":
                rprint(f"ℹ️ [yellow]Пользователь '{username}' не является администратором[/yellow]")
                return True
            
            if Confirm.ask(f"📉 Снять права администратора у пользователя [bold]{username}[/bold]?"):
                user.role = "user"
                await session.commit()
                rprint(f"✅ [green]Пользователь '{username}' больше не администратор[/green]")
                return True
            else:
                rprint("ℹ️ [yellow]Операция отменена[/yellow]")
                return False
...
def userinfo(username: str):
    ...
    async def _get_user_info():
        async with async_session_maker() as session:
            ...
            table.add_row("Статус", f"{status_emoji} {'Активен' if user.is_active else 'Неактивен'}")
